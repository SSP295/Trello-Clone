  ...
  return (
    <div className="relative w-full max-w-md animate-fadeIn">
      <div className="relative">
        <FiSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 transition-all duration-200 group-hover:scale-110" size={18} />
        <input
          type="text"
          value={query}
        ...
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-200 max-h-96 overflow-y-auto z-50 animate-scaleIn">
          {results.map((card, index) => (
            <button
              key={card.id}
              onClick={() => handleCardClick(card as CardType)}
            ...
            >
              <div className="font-medium text-gray-800 group-hover:text-blue-600 transition-colors duration-200">{card.title}</div>
              <div className="text-sm text-gray-500 mt-1">
                in {card.list?.title || 'Unknown'} • {card.list?.board?.title || 'Unknown Board'}
              </div>
            </button>
          ))}
        </div>
